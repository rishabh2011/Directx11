...
//Lighting functions
float4 calculateDirLight(DirectLight l, Material m, float3 normal, float3 viewDirW, float4 texColor);
float4 calculatePointLight(PointLight l, Material m, float3 normal, float3 viewDirW, float4 texColor);
float4 calculateSpotLight(SpotLight l, Material m, float3 normal, float3 viewDirW, float4 texColor);
...
float4 pixelShader(VertexOUT vout) : SV_TARGET
{
    ...
    if (useTexture)
    {
        texColor = diffuseMap.Sample(sam, vout.texCoords);
        //alphaTexColor = alphaMap.Sample(sam, vout.texCoords);
    }
    texColor = texColor; // * alphaTexColor;
...
//----------------------------------------------------------------------------------------------------
float4 calculateDirLight(DirectLight l, Material m, float3 normal, float3 vertexPosW, float4 texColor)
{
...
//-----------------------------------------------------------------------------------------------------
float4 calculatePointLight(PointLight l, Material m, float3 normal, float3 vertexPosW, float4 texColor)
{
...
//---------------------------------------------------------------------------------------------------
float4 calculateSpotLight(SpotLight l, Material m, float3 normal, float3 vertexPosW, float4 texColor)
{
    float3 lightDir = l.position - vertexPosW;
    float d = length(lightDir);
    ...
    return spotFactor * (texColor * (ambient + diffuse) + specular);
}